...
const HeaderComponent = ({isHiddenSearch =  false, isHiddenCart = false}) => {
    ...
    const content = (
        <div>
          ...
          <WrapperContentPopup onClick={() => handleClickNavigate('profile')}>INFORMATION</WrapperContentPopup>
  ...
  return (
    <div>
       <WrapperHeader style={{justifyContent: isHiddenSearch && isHiddenCart ? 'space-between' : 'unset'}}>
            <Col span={4}><a href='/'><WrapperTextHeader>Manga King</WrapperTextHeader></a></Col>
            ...
            <Col span={7} style={{display: 'flex', gap: '38px', alignItems: 'center'}}>
                        ...
                        {userdulieu?.access_token ? (
                            <>
                                <Popover content={content}  trigger="click" open={isOpenPopup}>
                                    <div style={{ cursor: 'pointer' }} onClick={() =>setIsOpenPopup((prev) => !prev)}> {userName?.length ? userName : userdulieu?.email}</div>
                                </Popover>
                            </>
                        ) : (
                            <div>
