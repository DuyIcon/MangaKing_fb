...
const OrderAdmin = () => {
  ...
  const dataTable = orders?.data?.length && orders?.data?.map((order) => {
    ...
    return { ...order, key: order._id, userName: order?.shippingAddress?.fullName, product:order?.orderItems[0]?.name, amount:order?.orderItems[0]?.amount ,phone: order?.shippingAddress?.phone, address: order?.shippingAddress?.address, paymentMethod: orderContant.payment[order?.paymentMethod],isPaid: order?.isPaid ? 'TRUE' :'FALSE',isDelivered: order?.isDelivered ? 'TRUE' : 'FALSE', totalPrice: convertPrice(order?.totalPrice)}
  })
